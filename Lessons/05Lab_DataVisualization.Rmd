...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025, # define text location
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))
    )
#using expression function: becasue there is a Greek letter in the equation 
#paste(): gluing texts together
...
#Save the plot
ggsave(
  filename = "./Output/Plot_text.jpg", #save the plot in EDE_Fall2023 folder 
  plot = Plot_text, #name of the plot 
  height = 4, 
  width = 6, 
  units = "in", 
  dpi = 300
...
ggplot(PeterPaul.chem.nutrients)+
  geom_bar(aes(x=factor(PeterPaul.chem.nutrients$month,levels=1:12, labels=month.abb)))+
  # convert month to a factor -- with 12 levels, labelled with month names
  # convert month column into a factor into the ggplot
  # month.abb: month abbreviation format 
  scale_x_discrete(name="month",drop=FALSE)
  # drop=FALSE: to include missing value 
...
#Apply a theme: to change the theme 
surface_temp_1999_plot + 
  theme_gray()
...
surface_temp_1999_plot + 
  ...
  labs(color='Lake') + #to change the name of the legend  
  theme_wsj()
```


```{r Legend Elements}
#Create a custom theme: theme()
my_theme <- theme_base() + 
  theme(
    line = element_line(
      color='black',
...
#plot)name+mytheme -> expand the theme in the environment 
surface_temp_1999_plot+my_theme
...
# Color your points by year.(color code -> the argument should be withing the aes)
# Change the ylab name

#surface: filter depth=0
surface_PeterPaul<-PeterPaul.chem.nutrients %>%
  filter(depth==0)
head(surface_PeterPaul)

ggplot(surface_PeterPaul)+
  geom_point(aes(x=daynum,y=temperature_C,color=year4))+
  facet_wrap(vars(lakename))+ 
  # facet by lake in two rows: facet_wrap(vars())
  ggtitle("Surface temperature")+
  labs(y="Surface Temperature")
  #error:as.factor(year4)
  

#2. 
# Plot temperature by date (x). Color your points by depth.
...
ggplot(surface_PeterPaul)+
  ...
  #geom_point(size=0.5): change the size of the points 
...
ggplot(EPAair)+
  geom_point(aes(x=PM2.5,y=Ozone,color=mean_lat)+
