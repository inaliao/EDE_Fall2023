...
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, lakename:irradianceDeck)
  
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul,lakename, year4, daynum, month, sampledate, depth:po4)
```
```{r}
# write a more succinct line of code to subset the nutrient dataset. 
NTL.nutrient.data.PeterPaul.subset2 <-NTL.nutrient.data.PeterPaul %>%
  select(tn_ug:po4) 

view(NTL.nutrient.data.PeterPaul.subset2)
```
...
```
```{r}
NTL.nutrient.data.PeterPaul.dates_3 <- mutate(NTL.nutrient.data.PeterPaul.subset,month=month(sampledate),year=year(sampledate),date=day(sampledate))
View(NTL.nutrient.data.PeterPaul.dates_3)
```
