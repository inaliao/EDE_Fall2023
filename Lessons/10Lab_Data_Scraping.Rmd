...
#Get the data, which starts on line 30 (skip=29)
#inseert a header: header=TRUE
#question: sep='\t'
gage_data <- read.table(theURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = TRUE) 
...
gage_data <- gage_data %>% 
  ...
  mutate(datetime = ymd_hm(datetime)) #ymd_hm: year month day, hour minute
...
#set the URL
theURL_5070<- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#start reading the data on line 30 
gage_data_5070<-read.table(theURL_5070,skip=30,header=TRUE,sep='\t',stringsAsFactors = TRUE)

#Update the column headers
colnames(gage_data_5070) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
gage_data_5070 <- gage_data_5070 %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime)) #ymd_hm: year month day, hour minute

#Plot
ggplot(gage_data_5070,aes(x=datetime,y=discharge_cfs)) + 
...
end.date = Sys.Date() #Gets today's date: Sys.Date()

#Load in data using the package's "readNWISdv" function
#readNWISdv(): to import data from NWIS web service
neuse <- readNWISdv(siteNumbers = siteNo, 
                    parameterCd = pcode, #discharge values 
                    statCd = scode, #mean daily discharge value 
                    startDate=start.date, 
                    endDate=end.date)
colnames(neuse)

#Rename columns to something more useful: renameNWISColumns()
#replace "X_00060_00003" and "X_00060_00003_cd" with "Flow" and "Flow_cd"  
#question: why
...
census_api_key("098b44a7a1081ff939eee585b4eabb613dfb7735", install = TRUE,overwrite=TRUE)

#obtain data and feature geometry from the American Community Survey: get_acs()
...
ggplot(nc_pop, aes(fill = estimate)) +
  geom_sf() +
  coord_sf(crs = 4326) + #crs=4326: projected coordinate reference system
  scale_fill_viridis(option = "viridis") #set the filled color
...
df_elec<-data.frame("states"=the_states,
                    "retail price (cent/kWh)"=as.numeric(the_price),
                    "net summer capacity (MW)"=as.numeric(gsub(",","",the_capacity)),
                    "net generation (MWh)"=as.numeric(gsub(",","",net_generation)), 
                    "retail sales (MWh)"=as.numeric(gsub(",","",total_retail))) 
#note: gsub()
...
scrape.it <- function(the_year){
  ...
  df_elec_function<-data.frame("states"=the_states,
                               "retail price (cent/kWh)"=as.numeric(the_price),
                                "net summer capacity (MW)"=as.numeric(gsub(",","",the_capacity)),
                                "net generation (MWh)"=as.numeric(gsub(",","",net_generation)), 
                                "retail sales (MWh)"=as.numeric(gsub(",","",total_retail))) 
  ...
  #Add year column
  df_elec_function['year']=my(paste0('1-',the_year))

  #Return the dataframe
  return(df_elec_function)
}
