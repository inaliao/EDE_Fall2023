...
H0: mu >= 50
Ha: mu < 50
also explain the t-test

```{r}
summary(EPAair$Ozone) #mean = 40.88 and median = 40
length(EPAair$Ozone) #8976 observations

#t.test(df, mu=, alternative="greater/less")
O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  geom_density(fill = "gray") +
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) #adding a vertical line at 50  +
  scale_x_continuous(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0)) # the distribution will fall above the x-axis 
print(Ozone.plot)
...
# Format as a GLM
# research question: is year a predictor of ozone 
