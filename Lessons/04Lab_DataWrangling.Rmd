...
#alternative:NTL.phys.data$sampledate<-as.Date(NTL.phys.data$sampledate,format="%Y/%m/%d")
...
NTL.phys.data_Tue_19_July<-NTL.phys.data %>%
  filter(year4>=1990 &year4<=1999) %>%
  filter(lakename=="Tuesday Lake") %>%
  mutate(month(sampledate)) %>%
  filter(month(sampledate)==6)
NTL.phys.data_Tue_19_July
...
NTL.phys.data_gather <- NTL.phys.data %>%
  pivot_longer(irradianceWater:irradianceDeck, names_to="Irradiance_Type",values_to="Irrandiance_Value",)
NTL.phys.data_gather 
 
#Exercise 6: Spread temperatureC into more than one column based on the depth.
NTL.phys.data_spread <- NTL.phys.data_gather %>%
