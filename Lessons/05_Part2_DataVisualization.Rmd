...
#sdefine the theme
mytheme <- theme_classic(base_size = 14) #to increase font size
...
theme_set(mytheme)#setting the theme within the plot

O3plot6 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot6)  

#because theme is set beforehand (theme_set(mytheme)) 
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  #define three different levels with horizontal lines
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #to justify dates
    date_breaks = "2 months", date_labels = "%b %y") +
  #%b: month in abbreviation format
  ylab(expression("O"[3]* " AQI Value")) +
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))
#change the angle of the labels so they will not overlap
print(O3plot7)  
...
NvsP2 <-
  ...
  #labs(): change your legend labels 
  scale_shape_manual(values = c(15, 17)) +
  #scale_color_distiller(palette = "Blues", direction = 1) +   #scale_shape_manual(values): change the shape of points
...
#rel_heights(): adjust the size of the plots
```
