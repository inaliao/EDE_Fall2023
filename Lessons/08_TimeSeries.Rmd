...
3. An **error** or **random** component, which makes up the remainder of the time series after other components have been accounted for. This component reflects the noise in the dataset (can no be explained by seasonality or trend).
...
# s.window="periodic" or the span (in lags) of the loess window for seasonal extraction
...
### Monotonic trend analysis: only has one direction in the trend (increasing or decreasing)

check the missing data first 
...
# adding new column with no missing observations (in real applications you will simply replace NAs)
#instead of using na.omit, using na.approx to estimate 

wind_data_clean <- 
  wind_data %>% 
  mutate(AWND.clean = zoo::na.approx(AWND))
# mutate (new_column=zoo::na.approx(column))
# use zoo:: to specify the function in zoo package

summary(wind_data_clean$AWND.clean)
# result: the NA is gone
...
* **Augmented Dickey Fuller (ADF test)**: no seasonality, non-parametric, missing data not allowed. Function: `adf.test()` (package: tseries)
