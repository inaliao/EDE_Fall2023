...
```{r}
#lm(y, x); lm(data=df, y~x)
#y: explanatory variable; x: dependent variable
...
>  
1. residual ranges from -458.9 to 23813.0
2. the slope of the regression line is -95.89, meaning there is a negative relationship between irradiance and depth level
3. standard error: the smaller the better 
4. p-value < alaph, meaning that the coefficient is statistically significantly different from 0, so it is worthwhile to test irradiance based on depth information 
5. r-square value indicates the value of depth interpreted by the value of irradiance 
...
1. **Residuals vs. Fitted.** The value predicted by the line of best fit is the fitted value, and the residual is the distance of that actual value from the predicted value. By definition, there will be a balance of positive and negative residuals. Watch for drastic asymmetry from side to side or a marked departure from zero for the red line - these are signs of a poor model fit.(the red line should be as flat as possible)

2. **Normal Q-Q.** The points should fall close to the 1:1 line (the dash line). We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. (compared the residuals to the normal distribution)
...
# remember to set the format of the figure in the code chunk 
# plot 4 polts in one plot 
# mfrow = c(2,2): divide the screen by 2*2
# mar=c(4,4,4,4): define the size of the plot 
...
```{r, fig.height = 3, fig.width = 4}
# remove the values of 0 and over 5000
...
# the residuals are expected to be smaller because the data was transferred (to log)
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") # add a line for the linear regression  +
  scale_y_log10() # y-axis will be the log-transformed data +
  geom_point() 
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black")  # add a line for the linear regression  
print(irradiancebydepth2)
...
> 
1. the p-value < alaph (confident level), indicating that intercept is statistically significantly different from 0 and depth is statistically different from 0
2. r-square increases, meaning that this model (with log data) is a better fit than the previous model
...
PeterPaulnutrients <- PeterPaul.chem.nutrients %>%
  select(tn_ug:po4) %>%
  na.omit()

#generate correlation plot metric 
...
overfitting: adding too many variables than actually need
...
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + temperature_C + tn_ug + po4)

# choose a model by AIC in a Stepwise Algorithm
step(TPAIC)
# removing the variable in the left-hand side and running the regression model again 
