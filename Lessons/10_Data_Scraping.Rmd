...
the_registrant <- webpage %>% 
  # grab the element using its tag
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% 
  # extract the value associated with the element text
  html_text()
the_registrant
```
...
#rep(starting value, ending value)
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! refers to variables
...
#use the year and the facility as inputs because those values change with URL
...
the_df2 <- scrape.it(2019,'0004-0001')
head(the_df2)
```
...
#Use lapply to apply the scrape function (will return a list of value)
the_dfs <- lapply(X = the_years, # a vector 
                  FUN = scrape.it, # the function to be applied to each element of X
                  the_facility=my_facility) # value
