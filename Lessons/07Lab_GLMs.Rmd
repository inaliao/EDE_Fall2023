...
#create a new data frame with only year and ozone
df_Ozone<-EPAair %>%
  select(Year,Ozone)
head(df_Ozone)

# density plot 
density_plot<-ggplot(data=df_Ozone)+
  geom_density(aes(x=Ozone,fill=as.factor(Year)),alaph=0.5)+
  scale_fill_manual(values=c("lightgreen","blue"))+
  scale_y_continuous(expand=c(0,0))+
  labs(x="Ozone",y="Density",fill="Year")+
  ggtitle("Ozone Levels")+
...
#box plot
box_plot<- ggplot(df_Ozone)+geom_boxplot(aes(x=as.factor(Year),y=Ozone))+
...
#stat_summary: adding a new layer to the plot, which represnet the max value in the plot 

#violin plot 
violin_plot<-ggplot(df_Ozone)+
  geom_violin(aes(x=as.factor(Year),y=Ozone),draw_quantiles = c(0.5))+
  stat_summary(geom="text",
               fun=max,
               vjust=-1,
               size=4,
...
scatter_plot<-ggplot(EPAair,aes(x=Ozone,y=PM2.5))+
