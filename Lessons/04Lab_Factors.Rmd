...
#enter here() on console: the project working directory 

#Read in data, don't convert strings to factors
sites <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_Raw.csv'))
dim(sites) #check the dimension

#Convert date column to date object
sites$date <- ymd_hms(sites$date)
sites$date
#ymd_hms: year month date, hours minutes second
#sites$date <- ymd(sites$date) : this will show N/A
...
sites$dow
#wday: compute the day of the week each sample was collected
...
unique(sites$dow)
#unique(df$column): the unique values in the column 
...
sites$dow
#factor(df$column, levels=):convert a vector into a factor (why)
#taking the unique values in the column as the same group -> beneficial for visualization 
...
#List the unique values in the column
unique(sites$dow)
#level 3, 4, 5: unique values that associated with the group
...
#add the levels associated with the factor 
...
#add one based on the existed levels 
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) #don't drop the levels that do not have the values 
