...
vector1[3] # the thrid element in vector1
```
```{r}
# Matrices ----
...
```
```{r}
matrix1[4, ] # print row 4
matrix1[ , 3] #print column 3
matrix1[c(12, 14)] # print element 12 and 14
matrix1[c(12:14)] # print from element 12 to 14
matrix1[2:4, 1:3] # row 2 to 4, column 1 to 3
```
```{r}
cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
...
# Data Frames ----
```{r}
d <- c(1, 2, 3, 4) # What type of vector? numaric 
...
> Answer:metrix and lists are listed as "data"; vectors are showed as "values" in the Environment tab. 

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer:show the name of the columns 
...
high_temp<-c(83,83,84,78,71,79,82,85,84,82,80,80,80,80);high_temp
low_temp<-c(56,58,60,61,63,63,64,64,63,60,59,58,57,57); low_temp
...
high_temp_c<-c((high_temp-32)*5/9);high_temp_c
low_temp_c<-c((low_temp-32)*5/9);low_temp_c
...
convert<-function(x){
  ...
  return(celcius)}
#error:return(list(celcius=celcius))

high_temp_cel<-convert(high_temp); high_temp_cel
low_temp_cel<-convert(low_temp); low_temp_cel
...
```{r}
df_temp_function<-cbind(high_temp_cel,low_temp_cel,high_temp, low_temp)
df_temp_function
...
#add column name 
#rename function
df_temp<-as.data.frame(cbind(high_temp, low_temp, high_temp_c,low_temp_c))
df_temp
```
...
```{r DataExercise-1d}
summary(df_temp)
sd(df_temp$high_temp)
#sd have to do with a specific vector 
```
...
fix.early.dates <- function(d, cutoff) {
       m <- year(d) %% 100  
       #operator %% is a modular division i.e. integer-divide year(d) by 100 and returns the remainder 
       year(d) <- ifelse(m > cutoff, 1900+m, 2000+m)  
       #this will update year(d), year() is a function that returns the year for a data object
       return(d)
}
