...
epa_pm25_sites_sf <- read_csv(here('Data/Raw/EPA/EPAair_PM25_NC2018_raw.csv')) %>% 
...
#read the data into spatial data frame 
...
#question: how to change the map legend: layer.name('name')
#question: how to change into projected coordinate reference system (instead of using mapview, use ggplot)
...
huc8_upper_sf<- st_read(here('Data/Spatial/NCHUC8.shp')) %>%
  filter(SUBBASIN=="Upper Neuse")

huc8_others_sf<- st_read(here('Data/Spatial/NCHUC8.shp')) %>%
  filter(SUBBASIN!="Upper Neuse")

#Create a map: use col.region to specify color 
# using ggplot 
ggplot()+
  geom_sf(data=huc8_sf, fill='orange')+
  geom_sf(data=huc8_upper_sf,fill='purple')

# using mapview 
map_upper<-mapview(huc8_upper_sf,col.regions='purple',layer.name='HUC8_upper_sf')
map_others<-mapview(huc8_others_sf,col.regions='orange',layer.name='HUC8_sf')
map_final<-map_others+map_upper
map_final
...
#merge the data from different data frame 
...
mapview(counties_sf_join, 
        zcol = 'E_POV',#colored by people in poverty
        col.regions = brewer.pal(2, 'RdBu'))+ #specify color ramp
  mapview(epa_pm25_sites_sf, cex = 'maxPM') # add another layer of map


#view with ggplot
ggplot() + 
  geom_sf(data=counties_sf_join,aes(fill = E_POV),alpha=0.3) + 
  scale_fill_gradient2(low="red",high="blue",midpoint = 60000) + #question: use mean as a midpoint?
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  colClasses = c('HUC_12' = 'factor')) %>%   #import column 'HUC_12' as a factor
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #extracting the first eighth number from the values in column 'HUC_12'
  group_by(HUC_8) %>% #remember to group before doing summary statistics
  summarize(SumWindEnergy = sum(AvgWindEnergy))
  
#Join to HUC_8 features
huc8_sf_join = merge(x = huc8_sf, y = wind_raw, by.x = 'HUC_8', by.y = 'HUC_8')
...
myMap = mapview(neuse_sf, 
                col.regions = 'yellow', 
                alpha.regions = 0.2, #change the transparent of the color 
                map.types = "CartoDB.Positron",
                legend = FALSE)
#Show the map
myMap

#approach 1: Select intersecting counties using matrix subsetting
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] 
# rows in the data frame
# [row, column]
# to see if each row meets the criteria or not 

#approach 2: Select intersecting counties using the `filter()` command
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE))
# spatial feature
#sparse: should a sparse index list be returned (TRUE) or a dense logical matrix (FALSE)?

#View the result: overlap two map layers 
...
# %in% provide the vector of the name we want 
# using == will need to list all the counties

#Grab the intersecting HUC_8s (metric selection)
# select all the hucs that have intersect
...
triCo_centroids_2km <- st_buffer(triCo_centroids, 2000) #2000 meter
myMap <- myMap + geom_sf(data = triCo_centroids_2km, color = 'orange', fill=NA)
myMap

#Buffer the counties inward 2km
triCo_in2km <- st_buffer(triCo, -2000) #buffer inside
