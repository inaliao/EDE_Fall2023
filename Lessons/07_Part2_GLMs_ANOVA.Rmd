...
#alternative method to test normality: plot normal Q-Q plot
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass)

# Test for equal variance: bartlett.test()
...
#results: reject null i.e. variances are not equal (p-value < confidence level)

# Format ANOVA as aov(data=df, continuous variable ~ catagorical variable)
...
# sum square: the deviation between the observation and the mean 
# F-value = mean square / mean of the residual 
...
# Post-hoc test: to explore differences between multiple group means 
# TukeyHSD() computes Tukey Honest Significant Differences
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships: to see which groups can be categorized as having the same mean
...
```{r}
# Run a post-hoc test for pairwise differences
# group the dataset based on the interaction effect
TukeyHSD(Litter.anova.2way3)

# create a new object that has 24 levels (24 levels is the result of functional group and class)
# with(df, interaction(column1, column2))
